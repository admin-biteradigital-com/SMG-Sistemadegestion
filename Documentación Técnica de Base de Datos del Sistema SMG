...
El siguiente diagrama representa el ERD completo del sistema SMG, incluyendo todas las entidades y relaciones.
erDiagram
    ...
    PROVEEDORES {
        ...
        varchar Contacto_Proveedor
        varchar Telefono_Proveedor
        varchar Email_Proveedor
        varchar Direccion_Proveedor
        varchar RUT_Proveedor UNIQUE
    }
    ORDENES_COMPRA {
        int ID_Orden PK
        date Fecha_Creacion
        ...
        datetime Fecha_Confirmacion_Proveedor
        text Notas_Proveedor_Confirmacion
        decimal Total_Orden
        text Notas
        int ID_Proveedor FK
    }
    PRODUCTOS_SERVICIOS {
        int ID_Producto_Servicio PK
        varchar Nombre_Producto_Servicio
        text Descripcion_Producto_Servicio
        decimal Precio_Unitario_Sugerido
        int Tiempo_Entrega_Proveedor_Dias
        int Stock_Seguridad_Minimo
        int Punto_Reorden
        int Cantidad_Reorden_Optima
        int ID_Unidad_Compra FK
        int ID_Unidad_Venta FK
        int ID_Unidad_Base FK
        varchar Codigo_Barras UNIQUE
        varchar Codigo_QR UNIQUE
    }
    DETALLES_ORDEN {
        int ID_Detalle_Orden PK
        int ID_Orden FK
        int ID_Producto_Servicio FK
        int Cantidad
        decimal Precio_Unitario_Acordado
    ...
    RECEPCIONES_MERCADERIA {
        ...
        varchar Nro_Guia_Remision
        text Observaciones
    }
    DETALLES_RECEPCION {
        int ID_Detalle_Recepcion PK
        int ID_Recepcion FK
        int ID_Producto_Servicio FK
        int Cantidad_Recibida
        date Fecha_Vencimiento
        varchar Numero_Lote
        int Cantidad_Recibida_Unidad_Base GENERATED ALWAYS AS (...) STORED
    }
    STOCK_DEPOSITO {
        int ID_Stock PK
        int ID_Producto_Servicio FK
        varchar Numero_Lote
        date Fecha_Vencimiento
        int Cantidad_Actual_Lote
        datetime Ultima_Actualizacion_Lote
    ...
    ORDENES_CARGA {
        int ID_Orden_Carga PK
        datetime Fecha_Carga
        int ID_Vehiculo FK
        int ID_Chofer FK
        varchar Estado_Carga
        text Observaciones
    }
    DETALLES_ORDEN_CARGA {
        int ID_Detalle_Orden_Carga PK
        int ID_Orden_Carga FK
        int ID_Producto_Servicio FK
        int ID_Peon_Carga FK
        int Cantidad_Cargada
        varchar Numero_Lote_Cargado
    }
    RUTAS {
        int ID_Ruta PK
        varchar Nombre_Ruta
        text Descripcion_Ruta
        decimal Distancia_Estimada_KM
        decimal Duracion_Estimada_Horas
    }
    CLIENTES {
        int ID_Cliente PK
        varchar Razon_Social
        varchar RUT_Cliente UNIQUE
        int Ciclo_Reabastecimiento_Dias
        decimal Limite_Credito_Autorizado
    ...
    SUCURSALES_CLIENTE {
        ...
        text Observaciones
        varchar Codigo_Sucursal UNIQUE NOT NULL
    }
    PEDIDOS_CLIENTE {
        int ID_Pedido PK
        int ID_Sucursal FK
        ...
        varchar Canal_Contacto
        varchar Estado_Pedido
        decimal Monto_Total_Estimado
        text Notas_Pedido
    }
    DETALLES_PEDIDO_CLIENTE {
        int ID_Detalle_Pedido PK
        int ID_Pedido FK
        int ID_Producto_Servicio FK
        int Cantidad_Solicitada
        int Cantidad_Entregada
        decimal Precio_Unitario_Acordado_Pedido
        decimal Subtotal_Linea_Pedido
        UNIQUE (ID_Pedido, ID_Producto_Servicio)
    }
    PEDIDOS_CARGADOS {
        int ID_Pedido_Cargado PK
        int ID_Orden_Carga FK
        int ID_Pedido FK
        int Cantidad_Cargada_Pedido
        UNIQUE (ID_Orden_Carga, ID_Pedido)
    }
    ORDENES_TRANSPORTE {
        int ID_Orden_Transporte PK
        int ID_Orden_Carga FK UNIQUE
        int ID_Ruta FK
        datetime Fecha_Salida
        datetime Fecha_Llegada_Estimada
        varchar Estado_Transporte
        text Observaciones
    }
    DESTINOS_TRANSPORTE {
        int ID_Destino_Transporte PK
        int ID_Orden_Transporte FK
        int ID_Sucursal FK
        int Orden_Visita
        varchar Estado_Entrega
        text Observaciones_Entrega
        int Tiempo_Estancia_Estimado_Minutos
        int Tiempo_Estancia_Real_Minutos
        UNIQUE (ID_Orden_Transporte, ID_Sucursal)
    }
    DETALLES_DESTINO_TRANSPORTE {
        int ID_Detalle_Destino PK
        int ID_Destino_Transporte FK
        int ID_Producto_Servicio FK
        int Cantidad_Entregar
    }
    ORDENES_VENTA {
        int ID_Orden_Venta PK
        int ID_Destino_Transporte FK
        datetime Fecha_Venta
        varchar Tipo_Documento_Venta
        decimal Monto_Total_Venta
        varchar Estado_Venta
        int ID_Agente_Venta FK
        int ID_Pedido FK
        varchar Metodo_Pago
        date Fecha_Vencimiento_Credito
        varchar Estado_Cobro
    }
    DETALLES_ORDEN_VENTA {
        int ID_Detalle_Venta PK
        int ID_Orden_Venta FK
        int ID_Producto_Servicio FK
        int Cantidad_Vendida
        decimal Precio_Unitario_Venta
        decimal Subtotal_Linea_Venta
    }
    FACTURAS {
        int ID_Factura PK
        int ID_Orden_Venta FK UNIQUE
        varchar Numero_Factura UNIQUE
        datetime Fecha_Emision
        decimal Monto_Neto
        decimal Monto_IVA
        decimal Monto_Total_Factura
        varchar Estado_Factura
        varchar Estado_SII
        varchar Folio_SII
        datetime Fecha_Envio_SII
    }
    PAGOS_RECIBIDOS {
        int ID_Pago PK
        int ID_Orden_Venta FK
        datetime Fecha_Pago
        decimal Monto_Pago
        varchar Metodo_Pago_Recibido
        date Fecha_Deposito_Efectivo
        varchar Numero_Documento_Pago
        text Notas_Pago
    }
    INTERACCIONES_CLIENTE {
        ...
        int ID_Cliente FK
        int ID_Sucursal FK
        datetime Fecha_Interaccion
        varchar Tipo_Interaccion
        text Notas_Interaccion
        int ID_Empleado FK
    }
    USUARIOS_CLIENTES {
        int ID_Usuario_Cliente PK
        int ID_Cliente FK UNIQUE
        int ID_Sucursal FK
        varchar Email_Usuario UNIQUE
        varchar Password_Hash
        datetime Fecha_Registro
        varchar Estado_Cuenta
    }
    RENDIMIENTO_EMPLEADO {
        int ID_Rendimiento PK
        int ID_Empleado FK
        varchar Periodo_Evaluacion
        date Fecha_Inicio_Periodo
        date Fecha_Fin_Periodo
        decimal Ventas_Totales_Periodo
        decimal Ventas_Credito_Cobradas_Periodo
        decimal Porcentaje_Cobranza_Periodo
        int Numero_Ordenes_Venta_Periodo
        int Numero_Rutas_Completadas_Periodo
        decimal Eficiencia_Ruta_Porcentaje
        decimal Comision_Calculada
        text Notas_Evaluacion
    }
    METAS_EMPLEADO {
        int ID_Meta PK
        int ID_Empleado FK
        varchar Tipo_Meta
        decimal Valor_Meta
        varchar Periodo_Meta
        date Fecha_Inicio_Meta
        date Fecha_Fin_Meta
    }
    CLIENTE_PREFERENCIAS {
        ...
        int Preferencia_Nivel
    }
    RUTAS_SUGERIDAS {
        int ID_Ruta_Sugerida PK
        datetime Fecha_Sugerencia
        int ID_Orden_Carga FK UNIQUE
        int ID_Vehiculo FK
        int ID_Chofer INT
        decimal Distancia_Optima_KM
        decimal Duracion_Optima_Horas
        text Notas_Optimizacion
    }
    DETALLES_RUTA_SUGERIDA {
        int ID_Detalle_Ruta_Sugerida PK
        int ID_Ruta_Sugerida FK
        int ID_Sucursal FK
        int Orden_Visita_Sugerida
